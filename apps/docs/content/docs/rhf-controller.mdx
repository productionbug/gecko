...
RHFController is a wrapper around React Hook Form's Controller component that automatically injects the `control` prop from `FormProvider`. This is the foundation for all RHF* components in HexPacket UI.

## Installation

```tsx
import { RHFController } from '@hexpacket/ui';
```
...
### With RHFController (HexPacket UI)
...
- Working with HexPacket UI components
