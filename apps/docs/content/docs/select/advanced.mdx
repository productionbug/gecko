...
import { SelectConsumerExample, ProgrammaticControlExample, ConditionalRenderingExample, CustomActionsExample, DebugViewExample } from '@/components/examples/select/advanced-examples';
...
import { Select, SelectOption, SelectConsumer } from '@hexpacket/ui';
      ...
      <SelectConsumer
        render={({ open, keyword, options }) => (
          <div className="p-3 border rounded-md bg-gray-50 text-sm mb-2">
            <p><strong>Status:</strong> {open ? 'Open' : 'Closed'}</p>
            <p><strong>Filter:</strong> {keyword || '(none)'}</p>
...
```tsx
import { useSelect } from '@hexpacket/ui';
