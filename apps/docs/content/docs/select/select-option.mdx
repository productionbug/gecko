...
<Select value={value} onChange={setValue}>
  ...
  <SelectOption value="admin-only" label="Admin Only" show="always" />
</Select>
```
...
| `show` | `'default' \| 'always' \| 'empty' \| 'filtered-and-empty'` | `'default'` | Visibility control |
...
- `HPuiSelectOption` - Base option element
- `HPuiSelectOption--selected` - Applied when selected
- `HPuiSelectOption--focused` - Applied when focused
- `HPuiSelectOption--disabled` - Applied when disabled
