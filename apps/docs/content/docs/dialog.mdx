...
import { Dialog, Button } from '@hexpacket/ui';
...
```tsx
import { Dialog } from '@hexpacket/ui';
```
...
Dialog requires `<HexpacketUIContainer />` to be mounted in your app:
...
- `HPuiDialog` - Main container
- `HPuiDialog__backdrop` - Background overlay
- `HPuiDialog__dialog` - Dialog content wrapper
