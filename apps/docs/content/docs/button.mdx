...
- `HPuiButton` - Main button class
- `HPuiButton--{variant}` - Variant modifier (filled, outlined, ghost, icon)
- `HPuiButton--{variant}-{color}` - Combined variant and color
- `HPuiButton--size-{size}` - Size modifier (xs, sm, md, lg, xl)
