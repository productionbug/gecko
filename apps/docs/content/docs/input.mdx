...
import { Input } from '@hexpacket/ui';
...
- `HPuiInput` - Main container
- `HPuiInput--enabled` - Applied when not disabled
- `HPuiInput--disabled` - Applied when disabled
- `HPuiInput--readonly` - Applied when read-only
- `HPuiInput__input` - The actual input element
