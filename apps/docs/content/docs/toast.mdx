...
import { toast } from '@hexpacket/ui';
...
```tsx
import { toast, HexpacketUIContainer } from '@hexpacket/ui';
```

## Setup

Add `HexpacketUIContainer` to your app root to enable toast notifications:
...
Configure toast defaults in `HexpacketUIContainer`:
