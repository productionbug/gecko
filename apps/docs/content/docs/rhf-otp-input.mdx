...
import { RHFOTPInput } from '@hexpacket/ui';
import {
...
import { z } from 'zod';
import { RHFOTPInput, RHFInputGroup } from '@hexpacket/ui';
...
- Red border via `HPuiRHFOTPInput--error` CSS class when `fieldState.error` exists
